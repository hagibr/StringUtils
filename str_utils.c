...
StringView sv_from_parts(const char *data, size_t len) {
  // Creates a view over an existing buffer slice — no copy, no null-terminator
  // needed
...
char *sv_to_cstr(StringView sv, char *buffer, size_t buf_len) {
  if (!buffer || buf_len < sv.len + 1)
    return NULL; // Need room for sv.len bytes + null terminator
  memcpy(buffer, sv.data,
         sv.len);        // Copy raw bytes — works even with embedded nulls
...
bool sv_equals_cstr(StringView a, const char *b) {
  if (!b)
    return false; // Treat NULL as not equal
...
StringView sv_substr(StringView sv, size_t start, size_t length) {
  if (start >= sv.len)
    return (StringView){sv.data + sv.len, 0};
...
StringView sv_split_next(StringView *input, char delim) {
  ...
  const char *end =
      memchr(start, delim, input->len); // Optimised delimiter search
  if (!end) {
    // Delimiter not found: return the whole remaining view and mark input as
    // consumed
  ...
  input->data = end + 1;
  input->len -= (token_len + 1); // Advance past the delimiter
  return token;
}
...
static bool sv_internal_to_u64(StringView sv, uint64_t *result, bool *neg) {
  if (sv.len == 0)
    return false; // Exit 1: Empty input
  size_t i = 0;
  uint64_t val = 0;
  int base = 10;
  *neg = false;
  ...
  if (i == sv.len)
    return false; // Exit 2: Input is just a sign or prefix with no digits

  bool has_digits = false;
  for (; i < sv.len; i++) {
    char c = sv.data[i];
    if (c == '_')
      continue; // Underscores are visual separators, skip them
    uint8_t digit;
    if (c >= '0' && c <= '9')
      digit = c - '0';
      ...
      return false; // Exit 3: Character is not valid for the current base

    if (digit >= (uint8_t)base)
      return false; // Exit 4: Digit value exceeds base (e.g. '2' in binary)
    if (val > (UINT64_MAX / (uint64_t)base))
      return false; // Exit 5: Next multiply would overflow
    val = (val * base) + digit;
    has_digits = true;
  }
  if (!has_digits)
    return false; // Exit 6: Only separators were found (e.g. "0x_")
  *result = val;
  return true;
}
...
bool sv_to_float64(StringView sv, double *res) {
  ...
  for (; i < sv.len; i++) {
    ...
    if (c == '_')
      continue; // Underscore separators are skipped (e.g. 1_000.50)
    ...
    if (!dot_seen) {
      ...
      val += (c - '0') *
             weight; // Fractional part: weight decreases with each position
      weight /= 10.0;
    }
    has_digits = true;
  }
...
bool sv_to_float32(StringView sv, float *res) {
  // Parse as double first, then check the value fits within float range
  // (±FLT_MAX)
  double v;
...
bool sv_hex_to_uint8(StringView sv, uint8_t *res) {
  if (sv.len == 0 || sv.len > 2)
    return false; // Only 1 or 2 hex chars map to a single byte
  uint32_t val = 0;
  for (size_t i = 0; i < sv.len; i++) {
    char c = sv.data[i];
    ...
    if (c >= '0' && c <= '9') {
      ...
      val |= (uint32_t)((c | 32) - 'a' +
                        10); // (c|32) lowercases without branching
    } else {
...
bool sv_hex_to_uint8_array(StringView sv, uint8_t *array, size_t *array_size) {
  if (!array || !array_size || sv.len % 2 != 0)
    return false; // Odd length would leave a dangling nibble
  ...
  for (size_t i = 0; i < needed; i++) {
    ...
    if (!sv_hex_to_uint8(b, &array[i]))
      return false;
  }
  *array_size = needed;
  return true; // Update caller with the actual number of bytes written
}
...
bool sv_parse_ipv4(StringView sv, uint8_t ip[4]) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (!sv_to_uint8(p, &ip[i]))
      return false;
  }
  return in.len ==
         0; // Fail if there are leftover characters after the 4th octet
}

bool sv_parse_mac(StringView sv, uint8_t mac[6]) {
  if (sv.len != 17)
    return false; // "XX:XX:XX:XX:XX:XX" is always exactly 17 chars
  char delim =
      sv.data[2]; // Detect delimiter from the position after the first byte
  if (delim != ':' && delim != '-')
    return false;

  StringView in = sv;
  for (int i = 0; i < 6; i++) {
    if (i < 5 && sv.data[2 + (i * 3)] != delim)
      return false; // Enforce consistent delimiter
    StringView p = sv_split_next(&in, (i < 5) ? delim : '\0');
    if (!sv_hex_to_uint8(p, &mac[i]))
      return false;
  }
  return in.len == 0;
}
...
int shell_parse_line(StringView line, StringView argv[], int max_args) {
  ...
  while (argc < max_args && input.len > 0) {
    if (input.data[0] == '"') {
      input.data++;
      input.len--; // Skip the opening quote
      StringView arg =
          sv_split_next(&input, '"'); // Read until the closing quote
      argv[argc++] = arg;
      // Note: escaped quotes are not supported — that would require modifying
      // the buffer or allocating memory, both undesirable in an embedded
      // context
    } else {
      const char *start = input.data;
      size_t l = 0;
      while (l < input.len && !isspace((unsigned char)start[l]))
        l++; // Scan to next whitespace
...
StaticBuilder sb_init(char *buf, size_t size) {
  ...
  if (size > 0)
    sb.data[0] =
        '\0'; // Ensure the buffer is a valid empty C-string immediately
  return sb;
}
...
void sb_reset(StaticBuilder *sb) {
  sb->len = 0;
  if (sb->capacity > 0)
    sb->data[0] = '\0'; // Reuse the same buffer from the start
}
...
bool sb_append_cstr(StaticBuilder *sb, const char *s) {
  return sb_append_sv(
      sb,
      sv_from_cstr(
          s)); // Thin wrapper — delegates length calculation to sv_from_cstr
}

bool sb_append_char(StaticBuilder *sb, const char c) {
  if (sb->len + sizeof(c) + 1 > sb->capacity)
    return false; // +1 for the null terminator
  sb->data[sb->len] = c;
  sb->len += 1;
  sb->data[sb->len] = '\0'; // Always keep the buffer null-terminated
...
bool sb_append_fmt(StaticBuilder *sb, const char *fmt, ...) {
  ...
  va_end(args);
  // vsnprintf returns the number of chars it would have written; if >=
  // remaining space, it truncated
  if (res < 0 || (size_t)res >= (sb->capacity - sb->len))
    return false;
  sb->len += (size_t)res;
  return true;
}
...
StringView sb_to_view(const StaticBuilder *sb) {
  return (StringView){
      sb->data, sb->len}; // The view shares the builder's buffer — no copy
}
...
static bool match_star(char c, StringView pattern, StringView text) {
  do {
    // Try to match the rest of the pattern against the current text position
    // (zero occurrences of c)
    if (match_here(pattern, text))
      return true;

    // Stop if text is exhausted or the current char doesn't match c ('.'
    // matches anything)
    if (text.len == 0 || (c != '.' && *text.data != c))
      break;
...
static bool match_here(StringView pattern, StringView text) {
  if (pattern.len == 0)
    return true; // Pattern exhausted — everything matched

  // '*' quantifier: delegate to match_star with the char before '*' and the
  // rest of the pattern
  if (pattern.len >= 2 && pattern.data[1] == '*') {
    return match_star(pattern.data[0],
                      (StringView){pattern.data + 2, pattern.len - 2}, text);
  }
  ...
  // '.' matches any single character; otherwise require an exact character
  // match
  if (text.len > 0 &&
      (pattern.data[0] == '.' || pattern.data[0] == *text.data)) {
    return match_here((StringView){pattern.data + 1, pattern.len - 1},
                      (StringView){text.data + 1, text.len - 1});
  }
...
bool sv_match(StringView pattern, StringView text) {
  ...
  if (pattern.len > 0 && pattern.data[0] == '^')
    return match_here((StringView){pattern.data + 1, pattern.len - 1}, text);
  ...
  while (true) {
    if (match_here(pattern, text))
      return true;
    if (text.len == 0)
      break; // No more positions to try
    text.data++;
    text.len--;
  }
  return false;
...
ByteStream bs_init(char *buf, size_t size) {
  // One slot is sacrificed as a sentinel so head==tail unambiguously means
  // empty
...
void bs_reset(ByteStream *bs) {
  bs->head = bs->tail =
      0; // Both cursors back to zero — buffer contents are abandoned
}

size_t bs_readable(const ByteStream *bs) {
  // Derived from head and tail only — no shared 'len' field, safe for
...
size_t bs_writable(const ByteStream *bs) {
  return bs->capacity -
         bs_readable(
             bs); // Usable capacity is always one less than the buffer size
}

size_t bs_write(ByteStream *bs, const char *data, size_t len) {
  size_t n =
      len < bs_writable(bs) ? len : bs_writable(bs); // Clamp to available space
  for (size_t i = 0; i < n; i++) {
    bs->buf[bs->head] = data[i];
    bs->head = (bs->head + 1) %
               (bs->capacity + 1); // Wrap head around the physical buffer
  }
  return n;
}
...
size_t bs_peek(const ByteStream *bs, char *dst, size_t len) {
  size_t n =
      len < bs_readable(bs) ? len : bs_readable(bs); // Clamp to available data
  for (size_t i = 0; i < n; i++) {
    dst[i] = bs->buf[(bs->tail + i) %
                     (bs->capacity + 1)]; // Wrap-aware read, tail is not moved
  }
  return n;
}
...
size_t bs_read(ByteStream *bs, char *dst, size_t len) {
  size_t n =
      bs_peek(bs, dst, len); // Reuse peek to avoid duplicating wrap logic
  bs->tail = (bs->tail + n) %
             (bs->capacity + 1); // Advance tail only after data is copied
  return n;
}
