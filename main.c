  ...
  while (1) {
    ...
    switch (sv_hash(args[0])) {
      ...
      case 0x3a0cb08e: // "?""
        ...
        printf("  int64, int32, int16, int8 <val>     : Parse signed integers "
              "(supports 0x, 0b, _)\n");
        printf("  uint64, uint32, uint16, uint8 <val> : Parse unsigned integers "
              "(supports 0x, 0b, _)\n");
        printf("  float64, float32 <val>              : Parse floating-point "
              "numbers\n");
        printf("  hex <string> [limit]                : Validate hex byte or "
              "parse byte array\n");
        printf("  ipv4 <address>                      : Validate IPv4 address "
              "string\n");
        printf("  mac <address>                       : Validate MAC address "
              "string\n");
        printf("  hash <string>                       : Calculate FNV-1a 32-bit "
              "hash\n");
        printf("  split <string> <char>               : Split string by a single "
              "delimiter\n");
        printf("  regex|match <pattern> <text>        : Match text against a "
              "simple regex pattern\n");
        printf("  build <args...>                     : Test StaticBuilder by "
              "joining arguments\n");
      ...
      case 0x03d22364: // "int64"
        ...
        int64_t v64;
        if (sv_to_int64(args[1], &v64))
          printf("Result: Valid int64 -> %lld\n", (long long)v64);
        else
          printf("Result: Invalid int64\n");
        break;

      case 0xfbdee2bf: // "int32"
        if (count < 2) {
          printf("Usage: int32 <value>\n");
          break;
        }
        int32_t v32;
        if (sv_to_int32(args[1], &v32))
          printf("Result: Valid int32 -> %d\n", v32);
        else
          printf("Result: Invalid int32\n");
        break;

      case 0x07e372d1: // "int16"
        if (count < 2) {
          printf("Usage: int16 <value>\n");
          break;
        }
        int16_t v16;
        if (sv_to_int16(args[1], &v16))
          printf("Result: Valid int16 -> %d\n", v16);
        else
          printf("Result: Invalid int16\n");
        break;

      case 0x6491fa92: // "int8"
        if (count < 2) {
          printf("Usage: int8 <value>\n");
          break;
        }
        int8_t v8;
        if (sv_to_int8(args[1], &v8))
          printf("Result: Valid int8 -> %d\n", (int)v8);
        else
          printf("Result: Invalid int8\n");
        break;

      case 0xaea00813: // "uint64"
        if (count < 2) {
          printf("Usage: uint64 <value>\n");
          break;
        }
        uint64_t u64;
        if (sv_to_uint64(args[1], &u64))
          printf("Result: Valid uint64 -> %llu\n", (unsigned long long)u64);
        else
          printf("Result: Invalid uint64\n");
        break;

      case 0x32940bec: // "uint32"
        if (count < 2) {
          printf("Usage: uint32 <value>\n");
          break;
        }
        uint32_t u32;
        if (sv_to_uint32(args[1], &u32))
          printf("Result: Valid uint32 -> %u\n", u32);
        else
          printf("Result: Invalid uint32\n");
        break;

      case 0xae8ebef2: // "uint16"
        if (count < 2) {
          printf("Usage: uint16 <value>\n");
          break;
        }
        uint16_t u16;
        if (sv_to_uint16(args[1], &u16))
          printf("Result: Valid uint16 -> %u\n", u16);
        else
          printf("Result: Invalid uint16\n");
        break;
      ...
      case 0x7c980e47: // "float64"
        if (count < 2) {
          printf("Usage: float64 <value>\n");
          break;
        }
        double f64;
        if (sv_to_float64(args[1], &f64))
          printf("Result: Valid float64 -> %lf\n", f64);
        else
          printf("Result: Invalid float64\n");
        break;

      case 0xe89f7410: // "float32"
        if (count < 2) {
          printf("Usage: float32 <value>\n");
          break;
        }
        float f32;
        if (sv_to_float32(args[1], &f32))
          printf("Result: Valid float32 -> %f\n", f32);
        else
          printf("Result: Invalid float32\n");
        break;
      ...
      case 0xfeb49d4a: // "hex"
        if (count == 2) {
          ...
          printf("Usage: hex <string> [limit]\n");
        break;

      case 0x285cc21e: // "ipv4"
        if (count < 2) {
          printf("Usage: ipv4 <addr>\n");
          break;
        }
        uint8_t ip[4];
      ...
      case 0x87b82de3: // "split"
        if (count < 3 || args[2].len != 1) {
          printf("Usage: split <str> <char>\n");
          break;
        }
      ...
      case 0x7E202F96: // "match"
        if (count < 3) {
          printf("Usage: regex|match <pattern> <text>\n");
          break;
        }
